...
const int ScriptParser::TYPE_NIL;
...
SymbolData *ScriptParser::buildSymbolTable(AST *theAST, map<string, long> *constants)
{
	...
	NilSymbols::getInst().addSymbolsToScope(globalScope,t);
    ...
    vid2 = globalScope->getVarSymbols().addVariable("nil", ScriptParser::TYPE_NIL);
    t->putVar(vid2, ScriptParser::TYPE_NIL);
    t->addGlobalPointer(vid2);
...
IntermediateData *ScriptParser::generateOCode(FunctionData *fdata)
{
    ...
    globalcode = NilSymbols::getInst().addSymbolsCode(lt);
